...
describe(`Vesting NFT Descriptor - (Unseen v${process.env.VERSION})`, async function () {
  ...
  function dyn(scheduleId: string): string {
    return `Unseen Vesting UNSEEN-VESTING #${scheduleId}`;
  }
  ...
  context('abbreviate amount', function () {
    ...
    it('tiny abbreviation amount', async function () {
      ...
      assert.equal(
        await aa(BigInt(1e17 - 1), 18),
        expectedAbbreviation,
        'abbreviation'
      );
      // assert.equal(
      //   await aa(BigInt(1e18 - 1), 18),
      //   expectedAbbreviation,
      //   'abbreviation'
      // );
    });
    it('zillions abbreviation amount', async function () {
      ...
      assert.equal(
        await aa(BigInt(1e15), 0),
        expectedAbbreviation,
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(1e16), 1),
        expectedAbbreviation,
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(1e16 + 1), 1),
        expectedAbbreviation,
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(1e17), 2),
        expectedAbbreviation,
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(1e17 + 1), 2),
        expectedAbbreviation,
        'abbreviation'
      );
      // assert.equal(
      //   await aa(BigInt(1e33), 18),
      //   expectedAbbreviation,
      //   'abbreviation'
      // );
      // assert.equal(
      //   await aa(BigInt(1e33 + 1), 18),
      //   expectedAbbreviation,
      //   'abbreviation'
      // );
      assert.equal(
    ...
    it('abbreviation amount thousands', async function () {
      ...
      assert.equal(
        await aa(BigInt(8211e18), 18),
        await ge('8.21K'),
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(201_287e18), 18),
        await ge('201.28K'),
        'abbreviation'
      );
    });
    it('abbreviation amount millions', async function () {
      ...
      assert.equal(
        await aa(BigInt(8_882_108e18), 18),
        await ge('8.88M'),
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(577_308_003e18), 18),
        await ge('577.30M'),
        'abbreviation'
      );
    });
    it('abbreviation amount billions', async function () {
      ...
      assert.equal(
        await aa(BigInt(9_927_800_422e18), 18),
        await ge('9.92B'),
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(699_881_672_021e18), 18),
        await ge('699.88B'),
        'abbreviation'
      );
    });
    it('abbreviation amount trillions', async function () {
      ...
      assert.equal(
        await aa(BigInt(9_621_312_102_753e18), 18),
        await ge('9.62T'),
        'abbreviation'
      );
      assert.equal(
        await aa(BigInt(101_076_479_280_188e18), 18),
        await ge('101.07T'),
        'abbreviation'
      );
    });
  ...
  context('name generation', function () {
    it('should generate name for empty input', async function () {
      expect(await gn('', '')).to.equal('Unseen Vesting  #', 'metadata name');
      expect(await gn('A', '')).to.equal('Unseen Vesting A #', 'metadata name');
      expect(await gn('', '1')).to.equal('Unseen Vesting  #1', 'metadata name');
    });
